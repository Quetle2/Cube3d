...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/03 15:38:03 by marada            #+#    #+#             */
/*   Updated: 2026/03/03 18:09:46 by marada           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pacima_helper(t_game *game, t_player *player)
{
	if ((player->key_up == 1)
		&& (ft_strchr("1A", game->map[(int)((player->y + 10
			...
			[(int)((player->x + (player->cos_angle * SPEED
						* player->gear)) / 64)]) != NULL
		|| ft_strchr("1A", game->map[(int)((player->y + 10
						+ (player->sin_angle * SPEED * player->gear)) / 64)]
			[(int)((player->x + 10 + (player->cos_angle * SPEED
						* player->gear)) / 64)]) != NULL
